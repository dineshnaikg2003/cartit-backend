...
public class OrderNumberGeneratorImpl implements OrderNumberGenerator {
	...
	public OrderNumberGeneratorImpl(OrderSequenceRepository repository) {
	...
	public synchronized String generateOrderNumber() {
		...
		OrderSequence sequence = repository.findBySequenceDate(today).orElseGet(() -> {
		...
		String runningNumber = String.format("%06d", sequence.getLastSequence());
