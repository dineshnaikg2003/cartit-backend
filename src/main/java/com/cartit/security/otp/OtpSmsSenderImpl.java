 ...
 * - Fast2SMS (Indian SMS gateway - Quick SMS route)
...
public class OtpSmsSenderImpl implements OtpSmsSender {
    ...
    public void sendOtp(String phone, String otp) {
        ...
        if (!enabled) {
            logSimulatedOtp(
                    cleanPhone,
                    otp,
                    "SMS sending is disabled via config (sms.enabled=false)"
            );
            return;
        }
            ...
            switch (provider.toLowerCase().trim()) {
                ...
                default:
                    logSimulatedOtp(
                            cleanPhone,
                            otp,
                            "Console provider selected"
                    );
                    break;
            }
        } catch (Exception e) {
            log.error(
                    "Failed to send OTP SMS to {}: {}",
                    cleanPhone,
                    e.getMessage(),
                    e
            );

            // Also log to console so admin/dev can still see the OTP
            // in development.
            logSimulatedOtp(
                    cleanPhone,
                    otp,
                    "Fallback due to SMS dispatch failure"
            );
        }
    }

    /**
     * Dispatches OTP using Fast2SMS Quick SMS Route.
     */
    private void sendFast2Sms(String phone, String otp) throws Exception {

        if (fast2smsApiKey == null
                || fast2smsApiKey.isBlank()
                || fast2smsApiKey.startsWith("YOUR_")) {
        ...
        String message =
                "Your CartIT verification OTP is "
                        + otp
         ...
         * {
         *   "route": "q",
         *   "message": "Your CartIT verification OTP is 123456. Valid for 5 minutes. Do not share it with anyone.",
         *   "numbers": "9876543210"
         * }
         */
        String jsonPayload = String.format(
                "{\"route\":\"%s\",\"message\":\"%s\",\"numbers\":\"%s\"}",
                fast2smsRoute,
                message,
                phone
        );
        ...
        if (response.statusCode() == 200) {
            log.info(
                    "Fast2SMS Quick SMS sent successfully to {}. Response: {}",
                    phone,
                    response.body()
            );
        } else {
            log.warn(
                    "Fast2SMS returned non-200 status {}: {}",
                    response.statusCode(),
                    response.body()
            );
            ...
            logSimulatedOtp(
                    phone,
                    otp,
                    "Fast2SMS error status: " + response.statusCode()
            );
        }
    }
    ...
    private void sendTwilioSms(String phone, String otp) throws Exception {
        ...
        if (twilioAccountSid == null
            ...
            logSimulatedOtp(
                    phone,
                    otp,
                    "Twilio credentials not configured in application.properties"
            );
            return;
        }

        String e164Phone = phone.startsWith("+")
                ? phone
                : "+91" + phone;
        ...
        if (response.statusCode() == 200
            ...
            log.info(
                    "Twilio SMS sent successfully to {}. SID response: {}",
                    e164Phone,
                    response.body()
            );
        } else {
            log.warn(
                    "Twilio returned non-200 status {}: {}",
                    response.statusCode(),
                    response.body()
            );
            ...
            logSimulatedOtp(
                    phone,
                    otp,
                    "Twilio error status: " + response.statusCode()
            );
        }
    }
    ...
    private void send2FactorSms(String phone, String otp) throws Exception {
        ...
        if (twoFactorApiKey == null
                || twoFactorApiKey.isBlank()
                || twoFactorApiKey.startsWith("YOUR_")) {

            logSimulatedOtp(
                    phone,
                    otp,
                    "2Factor API Key not configured in application.properties"
            );
            return;
        }

        String url = String.format(
                "https://2factor.in/v3/%s/SMS/%s/%s/CartIT_OTP",
        ...
        if (response.statusCode() == 200) {
            log.info(
                    "2Factor OTP sent successfully to {}. Response: {}",
                    phone,
                    response.body()
            );
        } else {
            log.warn(
                    "2Factor returned non-200 status {}: {}",
                    response.statusCode(),
                    response.body()
            );
            ...
            logSimulatedOtp(
                    phone,
                    otp,
                    "2Factor error status: " + response.statusCode()
            );
        }
    }
    ...
    private void logSimulatedOtp(
        ...
        System.out.println(
                " Message : Your CartIT verification code is " + otp + "."
        );
